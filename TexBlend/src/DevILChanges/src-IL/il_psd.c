...
ILboolean PsdGetData(PSDHEAD *Head, void *Buffer, ILboolean Compressed)
{
	...
	if (!Compressed) {
		...
		// Altered The following section to convert premultiplied alpha to straight alpha (removing the white alpha matte)

		float pxA;  // floats for premultiplied alpha correction
		float Cm;
		float Cu;
		ILboolean bUsePremutAlpha = ilIsEnabled(IL_USE_PREMULTALPHA);
		...
		if (Head->Channels >= 4) {
			// Removing multi-channel support to avoid extra precision loss from extra multiplies						
			//for (y = 0; y < Head->Height * iCurImage->Bps; y += iCurImage->Bps) {
			...
			for (; c < Head->Channels; c++) {
				...
				for (y = 0; y < Head->Height * iCurImage->Bps; y += iCurImage->Bps) {
					for (x = 0; x < iCurImage->Bps; x += iCurImage->Bpp, i++) {
						...
						//float newVal = ubyte_to_float(Channel[i]);
						//iCurImage->Data[y + x + 3] = float_to_ubyte(curVal * newVal);
						iCurImage->Data[y+x+3] = Channel[i];		// copy alpha channel over
						if(!bUsePremutAlpha && iCurImage->Data[y+x+3] != 255) {
							pxA = ubyte_to_float(iCurImage->Data[y+x+3]);

							Cm = ubyte_to_float(iCurImage->Data[y+x+0]);	// matted red component
							Cu = (Cm - (1.0f - pxA))/pxA;	//Cu = unmatted component. 
							iCurImage->Data[y+x+0] = float_to_ubyte(Cu);	

							Cm = ubyte_to_float(iCurImage->Data[y+x+1]);	// matted green component
							Cu = (Cm - (1.0f - pxA))/pxA;	//Cu = unmatted component. 
							iCurImage->Data[y+x+1] = float_to_ubyte(Cu);

							Cm = ubyte_to_float(iCurImage->Data[y+x+2]);	// matted Blue component
							Cu = (Cm - (1.0f - pxA))/pxA;	//Cu = unmatted component. 
							iCurImage->Data[y+x+2] = float_to_ubyte(Cu);	
						}
					}
				}
			}
		...
		/// End premultiplied alpha changed section
