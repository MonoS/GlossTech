...
ILAPI HBITMAP ILAPIENTRY ilutConvertSliceToHBitmap(HDC hDC, ILuint slice)
{
	...
	ILushort mattemode = 0;
	float Cb;	// matte component (1.0 for white, 0.0 for black)
	float Cu;	// unmatted component from image data
	float Cm;   // matted component value calculated
	float pxA;	// alpha value from image data
	float mv;	// matte component value
	...
	if (!alloc_buffer) {
		...
		// Configurable use of premultiplied alpha matting

		if(ilIsEnabled(IL_BMP_MATTEALPHA) && TempImage->Format == IL_RGBA) {
			mattemode = 1;
			Cb = 1.0f;
		}
		if(ilIsEnabled(IL_BMP_MATTEALPHAB) && TempImage->Format == IL_RGBA) {
			mattemode = 2;
			Cb = 0.0f;
		}
		...
		if (TempImage->Format == IL_RGB || TempImage->Format == IL_RGBA) {
			...
			for (j = 0; j < TempImage->Height; j++) {
				for (i = 0, n = 0; i < 3*TempImage->Width; i += 3, n += m) {
					if(mattemode) {  // bake alpha as a matte  according to http://dvd-hq.info/alpha_matting.php
						pxA = ubyte_to_float(TempImage->Data[k+n+3]);
						mv = (Cb * ( 1.0f - pxA ));

						Cu = ubyte_to_float(TempImage->Data[k + n + 2]);						
						Cm = ( Cu * pxA ) + mv;
						Data[l+i] = float_to_ubyte(Cm);

						Cu = ubyte_to_float(TempImage->Data[k + n + 1]);						
						Cm = ( Cu * pxA ) + mv;
						Data[l+i+1] = float_to_ubyte(Cm);

						Cu = ubyte_to_float(TempImage->Data[k + n]);						
						Cm = ( Cu * pxA ) + mv;
						Data[l+i+2] = float_to_ubyte(Cm);
