                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_TEXBLEND, szWindowClass, MAX_LOADSTRING);
	...
	AppController->InitTextureController();
	if(!AppController->OnAuto()) {
		AppController->SetBlankPreview();
		AppController->PopulateSetList();
	} else {
		return AppController->AutoHeadFix();
	}

	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_TEXBLEND));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_TEXBLEND));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_BTNFACE+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_TEXBLEND);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   hWnd = CreateWindowEx(0,szWindowClass, szTitle, WS_OVERLAPPEDWINDOW,
      CW_USEDEFAULT, 0, 730, 670, NULL, NULL, hInstance, NULL);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case MSG_PREVIEWCLOSING:
		AppController->NotifyPreviewClosing();
		break;
	case WM_NOTIFY:
